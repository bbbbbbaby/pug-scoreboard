-- ════════════════════════════════════════════════════════════════════
-- 008 — MOLTIPLICATORE PER-LAB
-- Eseguire in STAGING, testare, poi in PRODUZIONE. Idempotente.
-- Sostituisce il moltiplicatore globale: ora ogni lab ha il suo, scelto
-- alla creazione (colonna activities.lab_multiplier).
-- do_checkin aggiornata per leggerlo dal lab invece che dalla config.
-- ════════════════════════════════════════════════════════════════════

ALTER TABLE public.activities
  ADD COLUMN IF NOT EXISTS lab_multiplier numeric NOT NULL DEFAULT 2;
...
BEGIN
  ...
  SELECT lq.activity_id, a.name,
         ...
         coalesce(a.lab_multiplier, 2)  AS mult
    INTO v_lab
  ...
  IF FOUND THEN
    ...
    IF v_count >= v_lab.appointments THEN
      v_mult := v_lab.mult;  -- moltiplicatore del singolo lab
