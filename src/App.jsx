            ...
            <div className="pug-hero" style={{paddingTop:6}}>
              ...
              <div className="pug-roomzone">
                <div className="pug-room">
                  ...
                  {visConfig.creatura !== false && <button onClick={feedPet} disabled={feeding} title={"Dai da mangiare: " + (FOODS[selectedFood]?.name||"")} style={{position:"absolute",left:"50%",bottom:10,transform:"translateX(-50%)",background:"transparent",border:"none",padding:0,cursor:"pointer",lineHeight:0,transition:"transform .1s"}}>{FOODS[selectedFood]?.img ? <img src={FOODS[selectedFood].img} alt={FOODS[selectedFood].name} style={{width:58,height:58,objectFit:"contain",filter:"drop-shadow(0 3px 4px rgba(0,0,0,.45))"}}/> : <span style={{fontSize:30}}>{FOODS[selectedFood]?.emoji}</span>}</button>}
                </div>
